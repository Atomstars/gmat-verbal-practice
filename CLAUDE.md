...
python test_embeddings.py       # reads ALL THREE bank files, writes questions_embedded.json (~40s)
...
- `questions_embedded.json` is the merged ALL-BANKS index (generated by
  `test_embeddings.py`, which tags each record with `bank`: og|manhattan|quant)
  consumed by `api.py`. If you re-run any parser, regenerate it with
  `python test_embeddings.py` for `api.py` to pick up the changes.
...
| `GET /search-similar/{id}?limit=N&bank=X` | N nearest by cosine similarity; optional bank filter (og\|manhattan\|quant) |
| `GET /search?q=<text>&limit=N&bank=X` | Embed a freeform query on-the-fly; searches all banks unless filtered |
| `GET /questions/{id}` | Full question record (incl. `bank`, `diagram`) |
...
- **Vector features** (require `api.py` running; index covers ALL THREE banks):
  - **Similar questions panel** — rendered into `#simPanelSlot` (after `.pcard`) on every
    ...
    questions with type/difficulty/subtype pills; passes `&bank=App.source` so results
    are always openable in the current bank; clicking opens that question.
  - **Semantic search bar** (`#srchInput`) on the dashboard — 400ms debounce, calls
    `/search?q=...` (NO bank filter: searches all 910 questions), results in
    `#srchResults` with a bank pill on cross-bank hits; clicking a cross-bank result
    auto-switches `App.source` + reloads the bank, then starts a 1-question Practice
    session. Stems fall back to the API's `stem` field when the question isn't in the
    loaded bank. Controlled by `window.VECTOR_API`; silently absent when empty.
